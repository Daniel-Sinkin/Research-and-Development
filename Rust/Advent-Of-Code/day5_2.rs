...
impl Stacks {
    ...
    fn move_new(&mut self, n: usize, from: usize, to: usize) {
        let mut grip: Vec<char> = Vec::new();
        for _ in 0..n {
            grip.push(self.crates[from].pop().unwrap());
        }
...
fn main() -> io::Result<()> {
    ...
    if let Ok(mut file) = File::open(FILENAME) {
        ...
        for s in str.into_iter() {
            ...
            ms.move_new(n, from, to);
        }

        for i in 0..9 {
            print!("{}", ms.crates[i].last().unwrap());
        }
        println!();
    } else {
